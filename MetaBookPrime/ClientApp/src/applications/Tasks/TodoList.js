...
import { faCheck, faEdit, faPlusCircle, faTimes, faEye } from '@fortawesome/free-solid-svg-icons';
    ...
    ListItem(item) {
        ...
        if (!item.completed) {
            widget = <>
                <DetailModal
                    ...
                    block={false}
                    buttonLabel={<FontAwesomeIcon icon={faEye} />}
                >
                    <TodoItem
                        id={item.id}
                        item={item}
        ...
        return (
            <tr key={item.id}>
                ...
                <td className="align-middle">{item.dueDate ? dateFormatter(item.dueDate) : "None"}</td>
            </tr>
        );
    }
